...
public class Escape
{
	 ...
	 * @return 字符串
	 */
	public static String escape(String sValue) 
	{
		...
		for (int i = 0; i < len; i++) 
		{
			...
			else 
			{
				...
				stringBuilder.append(hex[(ch >>> 8)]);
				stringBuilder.append(hex[(0x00FF & ch)]);
			}
		}
	 ...
	 * @return 字符串
	 */
	public static String unescape(String sValue) 
	{
		StringBuilder stringBuilder = new StringBuilder();
		...
		while (i < len) 
		{
			...
			{
				int cint = 0;
				if ('u' != sValue.charAt(i + 1)) 
				{
					cint = (cint << 4) | val[sValue.charAt(i + 1)];
					cint = (cint << 4) | val[sValue.charAt(i + 2)];
				...
				{
					cint = (cint << 4) | val[sValue.charAt(i + 2)];
					cint = (cint << 4) | val[sValue.charAt(i + 3)];
					cint = (cint << 4) | val[sValue.charAt(i + 4)];
					cint = (cint << 4) | val[sValue.charAt(i + 5)];
